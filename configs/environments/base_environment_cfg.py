...
class EnvBaseConfig:
    """
    Базовая конфигурация для любой среды симуляции.

    Содержит общие параметры, такие как идентификатор, сид, настройки
    физики/времени и словарь для конфигураций гридов восприятия (`GridBaseConfig`).
    Конкретные конфигурации сред должны наследоваться от этого класса
    или включать его параметры для определения специфики среды.
    """

    device: str = "cuda" 

    env_id: str = "BaseEnv-v0"
    backend: Optional[str] = None  # 'mujoco', 'brax', 'genesis', etc.
    seed: int = 42
    max_episode_length: int = 1000

    # Общие параметры симуляции/управления
    simulation_frequency: int = 500  # Hz
    control_frequency: int = 50  # Hz
    # Количество шагов симуляции на один шаг управления (вычисляется)
    physics_steps_per_control_step: int = field(init=False, default=10)

    # Словарь для хранения конфигураций всех гридов этой среды
    # Ключ - имя грида (например, "front", "manipulator"), значение - объект GridBaseConfig
    grids: Dict[str, GridBaseConfig] = field(default_factory=dict)

    # Другие общие параметры (можно добавить по мере необходимости)
    # reward_config: Optional[Dict] = None
    # termination_config: Optional[Dict] = None
    # action_space_config: Optional[Dict] = None

    def __post_init__(self):
        """Вычисление зависимых параметров и проверка."""
        if self.simulation_frequency > 0 and self.control_frequency > 0:
            if self.simulation_frequency % self.control_frequency != 0:
                import logging

                logging.warning(
                    f"simulation_frequency ({self.simulation_frequency}) is not perfectly divisible by control_frequency ({self.control_frequency}). Using floor division."
                )
            self.physics_steps_per_control_step = max(
                1, self.simulation_frequency // self.control_frequency
            )
        elif self.control_frequency > 0:  # Если simulation_frequency не задана или 0
            self.physics_steps_per_control_step = (
                1  # Предполагаем 1 шаг симуляции на шаг управления
            )
        else:
            self.physics_steps_per_control_step = (
                1  # Или другое дефолтное значение / ошибка
            )
