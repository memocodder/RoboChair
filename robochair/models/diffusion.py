...
class DiffusionPolicy(nn.Module):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        лежащей в основе диффузионной моделью.
        """
        for key in batch:
            if key not in self._queues:
                continue
            if len(self._queues[key]) != self._queues[key].maxlen:
...
class DiffusionModel(nn.Module):
    ...
    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        if grid_data.ndim != 5:
            raise ValueError(
                f"Unexpected shape for '{grid_key}': {grid_data.shape}. Expected (B, S, C, H, W)."
            )
        grid_data_reshaped = einops.rearrange(
            grid_data, "b s c h w -> (b s) c h w"
        )
